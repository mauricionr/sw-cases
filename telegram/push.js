...
self.addEventListener('push', function(event) {
  ...
  var checksPromise = new Promise(function(resolve, reject) {
    ...
    Promise.all([getMuteUntil(), getLastAliveTime()]).then(function(result) {
      ...
      if (userInvisibleIsSupported() && muteUntil && nowTime < muteUntil) {
        console.log('Supress notification because mute for ', Math.ceil((muteUntil - nowTime) / 60000), 'min')
        return reject()
      }
      if (lastAliveTime && nowTime - lastAliveTime < 60000) {
...
self.addEventListener('message', function(event) {
  ...
  if (event.data.type == 'ping') {
    ...
    if (pendingNotification && client && 'postMessage' in client) {
...
(function() {
  ...
  var dbVersion = 2
  var openDbPromise
  var idbIsAvailable = self.indexedDB !== undefined && self.IDBTransaction !== undefined
